      ...
      <div className="w-full max-w-sm">
        <div className="p-8" style={{ background: "var(--color-surface)", border: "2px solid var(--color-divider)" }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                className="w-full px-3 py-2 text-sm" style={{ background: "var(--color-bg)", border: "2px solid var(--color-divider)", color: "var(--color-text)" }} placeholder="seu@email.com" />
            </div>
            <div>
              <label className="block text-xs mb-1" style={{ fontFamily: "var(--font-heading)", fontWeight: 800 }}>Senha</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required
                className="w-full px-3 py-2 text-sm" style={{ background: "var(--color-bg)", border: "2px solid var(--color-divider)", color: "var(--color-text)" }} placeholder="••••••••" />
            </div>
          ...
          <p className="text-xs mt-4 text-center" style={{ color: "var(--color-neutral-500)" }}>
            Não tem conta?{" "}
            <Link href="/cadastro" style={{ color: "var(--color-accent)", fontWeight: 800 }}>Criar conta</Link>
          </p>
        </div>
      </div>
    </div>
