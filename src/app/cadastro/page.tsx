        ...
        <div className="p-8" style={{ background: "var(--color-surface)", border: "2px solid var(--color-divider)" }}>
          ...
          {success ? (
            <div className="text-center">
              ...
              <p className="text-sm mb-2" style={{ color: "#155724", fontFamily: "var(--font-heading)", fontWeight: 800 }}>Conta criada!</p>
              <p className="text-xs mb-4" style={{ color: "#155724" }}>Agora é só fazer login.</p>
              <Link href="/login"><Button size="lg" className="w-full">Ir para login</Button></Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} required
                  className="w-full px-3 py-2 text-sm" style={{ background: "var(--color-bg)", border: "2px solid var(--color-divider)", color: "var(--color-text)" }} />
              </div>
              <div>
          ...
          <p className="text-xs mt-4 text-center" style={{ color: "var(--color-neutral-500)" }}>
            Já tem conta? <Link href="/login" style={{ color: "var(--color-accent)", fontWeight: 800 }}>Entrar</Link>
          </p>
        </div>
      </div>
    </div>
